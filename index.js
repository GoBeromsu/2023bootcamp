...
function problem1() {
    ...
    console.log(`평균나이: ${users.reduce((prev, cur) => prev + cur.age, 0)/users.length}`);
}

function problem2() {
    //TODO: 2. 24살이상인 user를 출력하시오
    console.log(users.filter((user) => user.age >= 24));
}
function problem3() {
    //TODO: 3. users의 이름[나이/성별]로 출력하시오
    //ex) 민준[24/남], 하윤[23/여]
    console.log(users.map((user) => `${user.name}[${user.age}/${user.gender == "male" ? "남" : "여"}]`));
}
...
function problem6() {
    ...
    console.log(users.sort((a, b) => new Date(a.birth).getTime() - new Date(b.birth).getTime()));
}
